...
function RecapulatifTheorique() {
    ...
    const getMoyenneP = (etudiantId, moduleId) => {
        try {
            ...
            if (note.CODEETUDIANT === etudiantId) {
                matieres.forEach(matiere => {
                    if (matiere.MODULEID === note.MODULEID) {
                        avg += note.TH * matiere.COEIFFICIENT
    ...
    return (
        <>
            <p className="fs-2 fw-bold text-white">RECAPILUTATIF DES EXMANES THEORIQUE</p>
            <RechercherEtudaint
...
export default RecapulatifTheorique
