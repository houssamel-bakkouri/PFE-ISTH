...
function RechercherEtudaint({ defaultNiveau, selectedAnnee, selectedNiveau, annees, setSelctedAnnee, filieres, setSelectedFiliere, niveaux, setSelectedNiveau, handleSearch }) {
    ...
    const handleFiliereChange = () => {
        if ((selectedNiveau === '' && !defaultNiveau) || selectedAnnee === '')
            return
        var f = [...filieres.current]
        if (defaultNiveau)
            f = f.filter(f => f.ANNEEID === parseInt(selectedAnnee) && f.NIVEAUID === parseInt(defaultNiveau))
        else
...
export default RechercherEtudaint
